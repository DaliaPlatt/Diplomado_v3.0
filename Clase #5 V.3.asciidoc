...
6. La transacción se transmite a través de la red a los **nodos**, quienes verificarán si es válida,
    - Después de ser verificada, permanece en una sala de espera hasta que un nodo minero la seleccione
7. Los **nodos mineros** seleccionan las transacciones con incentivos monetarios más atractivos 
    - Las agregan sus nuevos “bloques candidatos”, cada uno con su registro de transacciones por confirmar,
    - Comprimen toda la información creando un identificador de bloque.
...
Las transacciones son simplemente **entradas** y **salidas** de bitcoin de una dirección a otra
...
- Cuando un usuario desbloquea su UTXO con su clave privada para enviarle a otro,
    ...
    - Por este motivo, es recomendable siempre mandar cualquier saldo a una dirección nueva
- Si la dirección original tiene un saldo:
...
- las cajas grises representan direcciones en los que ya no hay bitcoin (completamente vacías).
- Diagrama #1:
...
Después de haber confirmado la transferencia, la blockchain solo se encargará de monitorear las direcciones que recibieron dinero, el de 1.5BTC, y el de 0.5BTC. 
